"""
User accounts: who may sign in, and how their passwords are checked.

Accounts live in one JSON file (``GBD_USERS_FILE``, default
``data/access/users.json``). It is git-ignored, never copied into an image, and
written with owner-only permissions. The same file serves every deployment:

* the local and server application check passwords against it;
* the GitHub Pages build uses it to seal the results, so that only these
  credentials can open the published copy (see ``scripts/build_static_site.py``).

Passwords are stored as PBKDF2-HMAC-SHA256 hashes -- 600,000 iterations and a
random 16-byte salt per user, OWASP's current recommendation for PBKDF2. PBKDF2
rather than Argon2 because browsers implement it natively: on GitHub Pages the
same derivation, run in the reader's browser, produces the key that opens their
copy of the results. A stored hash is therefore a secret. Treat the users file
like any password database.

The file is versioned so that moving accounts into a database -- with
self-registration and approval, for thousands of users -- keeps every existing
hash valid.
...
.. code-block:: console
    ...
    make user-add EMAIL=someone@example.org      # prompts for the password twice
    make user-list
    make user-remove EMAIL=someone@example.org
"""
...
# Allow `python app/accounts.py` as well as `python -m app.accounts`.
...
def parse_users(text: str) -> dict[str, User]:
    ...
    if data.get("format") != FORMAT or data.get("version") != VERSION or data.get("kdf") != KDF:
        raise ValueError("Not a version 1 gbd-users file")
    users = {}
    for item in data.get("users", []):
        ...
        if len(user.salt) < SALT_BYTES or len(user.hash) != HASH_BYTES or user.iterations < 100_000:
...
def load_users(path: Path | None = None) -> dict[str, User]:
    ...
    change -- a removed user, a new password -- takes effect on the next one.
    """
...
_DECOY_SALT = hashlib.sha256(b"gbd-accounts-decoy").digest()[:SALT_BYTES]
...
def password_version(user: User) -> str:
    ...
    return hmac.new(user.hash, b"gbd-session-version", hashlib.sha256).hexdigest()[:16]
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Manage who can sign in.")
    parser.add_argument(
        "--file", type=Path, default=None, help="Users file (default: GBD_USERS_FILE)"
    )
    ...
    try:
        if args.command == "add":
            ...
            print(f"Saved {user.email} to {path}")
            print(
                "GitHub Pages copy: update the repository secret GBD_USERS_JSON (make user-export)."
            )
        elif args.command == "remove":
            if not remove_user(args.email, path):
                print(f"No account for {args.email}", file=sys.stderr)
                return 1
            print(f"Removed {args.email}. Update GBD_USERS_JSON so the Pages copy is resealed.")
