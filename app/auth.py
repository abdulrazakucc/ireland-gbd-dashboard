...
``GBD_AUTH_MODE`` chooses how the application knows who is asking:

* ``password`` (default) -- email and password, checked against the users file
  (``app.accounts``). The same on a laptop and on a server.
* ``proxy`` -- an identity-aware reverse proxy (for example UCC single sign-on)
  authenticates people and passes their identity on.
...
session state is stored on the server, which keeps several server processes
behind a load balancer consistent -- as long as they share ``GBD_SESSION_SECRET``.
"""
...
def _key() -> bytes:
    ...
    # simply end when the app restarts. Production refuses to start without one.
...
class LoginLimiter:
    ...
    Kept in process memory, which is right for one server process. Running
    several processes for thousands of users needs a shared store (the accounts
    database or Redis) so that the limit holds across all of them.
    """
...
def session(request: Request) -> dict:
    """Who is signed in. Always 200, so the app can decide what to show."""
