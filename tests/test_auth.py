...
credentials also open the sealed GitHub Pages copy); results are unreachable
without a valid session; and sessions end when an account is removed or its
password changes.
"""
...
def fast_hashing(monkeypatch):
    ...
    monkeypatch.setattr(accounts, "ITERATIONS", 100_000)
...
@pytest.fixture()
def secure_client(seed_db, users_file, monkeypatch):
    ...
    with TestClient(create_app(db_path=seed_db)) as client:
        yield client
...
class TestPasswords:
    def test_hashing_is_standard_pbkdf2_sha256(self) -> None:
        """RFC 7914 / RFC 6070-style vector: what WebCrypto derives in the browser."""
    ...
    @pytest.mark.parametrize("weak", ["short", " padded password ", "presenter@example.org"])
    ...
    def test_authentication(self, users_file) -> None:
        ...
        assert accounts.authenticate("nobody@example.org", PASSWORD) is None
    ...
    def test_a_new_password_replaces_the_old_and_keeps_the_account(self, users_file) -> None:
        ...
        assert after.created_at == before.created_at and after.name == "Presenter"
    ...
    def test_weak_entries_in_the_file_are_rejected(self, users_file) -> None:
        text = users_file.read_text().replace('"iterations": 100000', '"iterations": 1000')
...
class TestSignIn:
    @pytest.mark.parametrize(
        "path", ["/api/meta", "/api/series", "/api/dimensions", "/api/ranked/options"]
    )
    def test_results_need_a_session(self, secure_client, path) -> None:
        assert secure_client.get(path).status_code == 401

    def test_downloads_need_a_session(self, secure_client) -> None:
        for path in ("/api/export.csv", "/api/figure.png", "/api/figure.pdf"):
            assert secure_client.get(path, params={"series": "x"}).status_code == 401, path

    def test_health_pages_and_session_status_stay_public(self, secure_client) -> None:
    ...
    def test_signing_in_opens_the_results(self, secure_client) -> None:
        response = _sign_in(secure_client)
        assert response.status_code == 200
        cookie = response.headers["set-cookie"].lower()
        assert "httponly" in cookie and "samesite=lax" in cookie
        assert secure_client.get("/api/series").status_code == 200
    ...
    def test_wrong_password_and_unknown_email_get_the_same_answer(self, secure_client) -> None:
        ...
        unknown = _sign_in(secure_client, email="nobody@example.org")
    ...
    def test_signing_out_ends_the_session(self, secure_client) -> None:
        ...
        assert secure_client.get("/api/series").status_code == 401
...
class TestProductionSafety:
    def test_production_refuses_to_run_without_sign_in(self, seed_db, monkeypatch) -> None:
        from app.main import create_app

        monkeypatch.setattr(config, "ENVIRONMENT", "production")
        monkeypatch.setattr(config, "AUTH_MODE", "off")
        with pytest.raises(RuntimeError, match="requires sign-in"):
            create_app(db_path=seed_db)

    def test_production_password_mode_needs_a_strong_session_secret(self, seed_db, monkeypatch):
        from app.main import create_app
        ...
        assert create_app(db_path=seed_db)
...
class TestCommandLine:
    def test_add_list_export_and_remove(self, tmp_path, monkeypatch, capsys) -> None:
        ...
        args = ["--file", str(path)]
        assert accounts.main([*args, "add", EMAIL, "--name", "Presenter", "--password-stdin"]) == 0
        assert accounts.main([*args, "list"]) == 0
        assert EMAIL in capsys.readouterr().out
        assert accounts.main([*args, "export"]) == 0
        exported = capsys.readouterr().out
        assert accounts.parse_users(exported)[EMAIL].name == "Presenter"
        assert hashlib.sha256(PASSWORD.encode()).hexdigest() not in exported
